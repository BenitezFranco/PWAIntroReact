...
function ContadorTareas({ tareas }) {
  ...
  const tareaCompletada = tareas
    ? tareas.filter((tareas) => tareas.completed).length
    : 0;
  ...
  if (totalTareas === 0) {
    //return mensaje1. Lo mismo para los demas
    ...
    <div>
      ...
      <ListGroup>
        <ListGroupItem className="justify-content-between">
          TOTAL DE TAREAS <Badge pill>{totalTareas}</Badge>
          TAREAS COMPLETADAS: {""}
          <Badge pill>
      ...
      {/* Nice feature! */}
      <Progress multi>
        <Progress animated bar color="success" value={porcentajeCompletado}>
          Tareas completadas
        </Progress>
